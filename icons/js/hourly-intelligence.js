...
// GENAU EIN Wetter-Request + EIN Pegel-Request (nur jetzt-nahe Stunden) fuer den GESAMTEN
// Horizont geladen, und jede Stunde danach LOKAL (ohne weiteren Netzwerkaufruf) aus den
// geladenen Arrays mit STRIKTEM Timestamp-Matching (_exactHourIndex — kein "naechstgelegener
// Wert", um stille Off-by-one-Fehler auszuschliessen) zusammengesetzt.
//
// HI-2A.1 HOTFIX (31.08.2026, echter Livetest-Befund): Wassertemperatur und Wellen sind seit
// diesem Hotfix ZWEI getrennte Requests/Methoden (getWaterTempRangeRaw/getWaveRangeRaw in
// providers.js) statt einer kombinierten getMarineRangeRaw() — der urspruengliche kombinierte
// Request erzwang ein Wellenmodell fuer den GESAMTEN Request (inkl. sea_surface_temperature),
// was SST fuer die Luebecker Bucht faelschlich auf null gesetzt hat, obwohl der Wassertemperatur-
// Provider selbst funktioniert (siehe getWaterTempRangeRaw()-Kommentar in providers.js fuer die
// vollstaendige Root-Cause-Analyse). Nutzt weiterhin dieselben reinen Formeln
// (computeSolarFeatures/classifyThermalRegime/analyzeWaterLevelPhase, NUR LESEND) wie
// buildHourlyEnvironment/-Features — keine zweite Logik, nur eine zweite Datenquelle.
// ---------------------------------------------------------------------------
...
function _mapForecastHour(waterId, lat, lon, targetDt, generatedAt, batchStartMs, weatherRange, waterTempRange, waveRange, waterLevelPoints) {
  const nowMs = Date.now();
  ...
  const environment = {
    ...
    // HI-2A.1 FIX (Auftrag Abschnitt 10): forecastHorizonHours beschreibt im Batch-Kontext den
    // Abstand der Zielstunde zum BATCH-START (h=0..horizonHours), NICHT die (ggf. negative)
    // abgerundete Differenz zur "jetzt"-Sekunde von generatedAt — sonst waere h=0 faelschlich
    // "-1 Stunden voraus", sobald generatedAt nicht exakt auf eine volle Stunde faellt (z.B.
    // generatedAt=11:44, startTimestamp=11:00 -> h=0 ist "0 Stunden ab Batch-Start", nicht "-1
    // Stunden ab 11:44"). Der bestehende HI-1-Einzelstunden-Pfad (buildHourlyEnvironment(), siehe
    // oben) ist davon NICHT betroffen — dort bleibt die bisherige "Abstand zu jetzt"-Semantik
    // unveraendert, da dort kein Batch-Start existiert.
    forecastHorizonHours: Math.round((targetMs - batchStartMs) / 3600000),
    source: "none",
  };
  ...
  // HI-2A.1: SST und Wellen sind GETRENNTE Anfragen/Arrays (Root-Cause-Fix, siehe
  // getWaterTempRangeRaw()/getWaveRangeRaw() in providers.js) — bewusst zwei unabhaengige
  // Timestamp-Matches, damit ein fehlender/nulled Wert in einer Quelle die andere NICHT
  // beeinflusst (automatisiert geprueft: "Wellen komplett null, waterTempC trotzdem befuellt"
  // und umgekehrt).
  if (waterTempRange && waterTempRange.ok) {
    const tIdx = _exactHourIndex(waterTempRange.times, targetMs);
    if (tIdx >= 0) {
      environment.waterTempC = _fieldAt(waterTempRange.waterTempC, tIdx);
      if (environment.waterTempC !== null) sources.push("open_meteo_marine_sst_batch");
    }
  }
  if (waveRange && waveRange.ok) {
    const vIdx = _exactHourIndex(waveRange.times, targetMs);
    if (vIdx >= 0) {
      environment.waveHeightM = _fieldAt(waveRange.waveHeightM, vIdx);
      environment.waveDirectionDeg = _fieldAt(waveRange.waveDirectionDeg, vIdx);
      environment.wavePeriodSec = _fieldAt(waveRange.wavePeriodSec, vIdx);
      if (environment.waveHeightM !== null) sources.push("open_meteo_marine_wave_batch");
    }
  }
...
//   horizonHours, hours: [{environment, features}, ...], requestLog,
//   waterTempSourceStatus, waterTempModel, waveSourceStatus, waveModel }
...
// HI-2A.1: SST (getWaterTempRangeRaw) und Wellen (getWaveRangeRaw) sind seit dem Hotfix ZWEI
// getrennte Requests statt einer kombinierten getMarineRangeRaw() — siehe providers.js-Kommentar
// fuer die Root-Cause-Analyse der urspruenglichen SST-Regression. Damit sind es fuer ein
// Kuestenprofil mit allen drei Providern jetzt 4 statt 3 Requests total (Auftrag Abschnitt 9:
// "3 Requests -> 4 Requests ist vollkommen akzeptabel, korrekte Daten sind wichtiger") — weiterhin
// GENAU je 1 Request pro Datenquelle fuer den GESAMTEN Horizont, keine stuendlichen Einzelrequests.
async function buildHourlyForecastSeries(waterId, opts = {}) {
  ...
  const result = { generatedAt, locationId: waterId, startTimestamp: startHour.toISOString(),
    horizonHours, hours: [], requestLog,
    waterTempSourceStatus: "request_failed", waterTempModel: null,
    waveSourceStatus: "request_failed", waveModel: null };
  ...
  let weatherRange = null, waterTempRange = null, waveRange = null, waterLevelPoints = null;
  ...
  if (profile.waterTempProvider && typeof profile.waterTempProvider.getWaterTempRangeRaw === "function") {
    waterTempRange = await profile.waterTempProvider.getWaterTempRangeRaw(lat, lon, rangeStart, rangeEnd);
    requestLog.push({ provider: profile.waterTempProvider.name, kind: "watertemp_range", ok: !!waterTempRange.ok });
    result.waterTempSourceStatus = waterTempRange.waterTempSourceStatus || (waterTempRange.ok ? "provider_null" : "request_failed");
    result.waterTempModel = waterTempRange.waterTempModel || null;
  }
  if (profile.waterTempProvider && typeof profile.waterTempProvider.getWaveRangeRaw === "function") {
    waveRange = await profile.waterTempProvider.getWaveRangeRaw(lat, lon, rangeStart, rangeEnd);
    requestLog.push({ provider: profile.waterTempProvider.name, kind: "wave_range", ok: !!waveRange.ok });
    result.waveSourceStatus = waveRange.waveSourceStatus || (waveRange.ok ? "provider_null" : "request_failed");
    result.waveModel = waveRange.waveModel || null;
  }
  ...
  const batchStartMs = startHour.getTime();
  for (let h = 0; h <= horizonHours; h++) {
    const targetDt = new Date(batchStartMs + h * 3600000);
    result.hours.push(_mapForecastHour(waterId, lat, lon, targetDt, generatedAt, batchStartMs, weatherRange, waterTempRange, waveRange, waterLevelPoints));
  }
  return result;
}
