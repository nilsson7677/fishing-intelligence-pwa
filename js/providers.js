...
// #1364 (Rotterdam-Fall): expliziten Modellnamen statt "best_match" anfordern.
//
// HI-2A.1 HOTFIX (31.08.2026, echter GitHub-Pages-Livetest): der urspruengliche HI-2A-Fix
// ("dwd_ewam", DWD EWAM, 5 km Aufloesung, NUR Europa) wurde live getestet und lieferte fuer den
// Luebecker-Bucht-Referenzpunkt weiterhin waveSourceStatus="provider_null" — das feine
// Europa-Gitter hat an diesem konkreten, schmalen/halb-eingeschlossenen Buchtpunkt offenbar
// keine belastbare Wasserzelle (strukturell derselbe Effekt wie der Rotterdam-Fall aus Issue
// #1364, nur mit einem ANDEREN Modell). ZUSAETZLICH deckte derselbe Livetest eine zweite, davon
// unabhaengige Regression auf: die alte getMarineRangeRaw() forderte
// "sea_surface_temperature,wave_height,wave_direction,wave_period" in EINEM Request MIT
// erzwungenem "models=dwd_ewam" an — dwd_ewam ist laut Open-Meteo-Doku ein reines Wellenmodell
// (DWD "European WAve Model"), OHNE dokumentierte sea_surface_temperature-Unterstuetzung
// (SST hat bei Open-Meteo eine eigene Datenquelle, z.B. "MeteoFrance SST", die nichts mit den
// Wellenmodellen zu tun hat). Ein einzelner Request, der SST UND Wellen unter einem erzwungenen
// Wellenmodell anfordert, liefert deshalb fuer SST ebenfalls null — bestaetigt durch den Livetest
// (waterTempC: 0/121, obwohl derselbe Referenzpunkt im HI-1-Einzelstunden-Livetest mit
// getWaterTemp() — KEIN "models"-Parameter, getrennter Request — 19.4-19.5 °C lieferte). Die
// Einzelstunden-Methoden getWaterTemp()/getWaveData() waren von diesem zweiten Fehler NICHT
// betroffen, da sie bereits als zwei getrennte Requests implementiert sind — nur die neue
// Batch-Methode kombinierte beides faelschlich in einem Request.
//
// FIX (HI-2A.1): (1) SST und Wellen sind jetzt AUCH im Batch-Pfad zwei vollstaendig getrennte
// Requests/Methoden (getWaterTempRangeRaw() OHNE "models"-Parameter, identisch zum bewiesenermassen
// funktionierenden getWaterTemp() — SST braucht KEIN erzwungenes Wellenmodell). (2) Das
// Wellenmodell wurde von "dwd_ewam" (live widerlegt) auf "ecmwf_wam" (ECMWF WAM, 9 km Aufloesung,
// GLOBAL statt nur Europa) umgestellt — EIN bewusst gewaehlter, dokumentierter Kandidat (nicht aus
// derselben DWD-Modellfamilie wie der bereits gescheiterte Versuch, globale statt regionale
// Abdeckung, im Auftrag HI-2A.1 Abschnitt 5 explizit als Beispielkandidat genannt) statt einer
// blinden Fallback-Kette. Auch dieser Fix ist SOURCED, aber NICHT live verifizierbar (Netzwerk in
// dieser Sandbox weiterhin blockiert, siehe Phase 6B) — naechster Livetest-Punkt.
const OM_MARINE_WAVE_MODEL = "ecmwf_wam";

// Modelle mit einer in der offiziellen Doku EXPLIZIT genannten geografischen Einschraenkung
// (nicht geraten — direkt aus der Modell-Tabelle der Open-Meteo-Marine-Doku uebernommen). Wird
// NICHT fuer OM_MARINE_WAVE_MODEL (ecmwf_wam, global, keine Einschraenkung) ausgeloest, ist aber
// generisch und testbar fuer den Fall, dass ein zukuenftiges Gewaesser-Profil ein regional
// eingeschraenktes Modell explizit waehlt (Auftrag Abschnitt 5: "outside_coverage" nur dann, wenn
// tatsaechlich belegt, kein Raten).
const OM_MARINE_MODEL_COVERAGE = {
  gfs_wave_016: { latMin: -15, latMax: 52.5 }, // Doku: "Latitude 52.5°N - 15°S"
};

function _isOutsideKnownCoverage(model, lat) {
  const c = OM_MARINE_MODEL_COVERAGE[model];
  if (!c) return false; // kein dokumentiertes Limit bekannt -> keine Behauptung, kein Raten
...
class OpenMeteoMarineProvider {
  ...
  async getWaveData(lat, lon, dt) {
    if (_isOutsideKnownCoverage(OM_MARINE_WAVE_MODEL, lat)) {
      return { ok: true, waveHeightM: null, waveDirectionDeg: null, wavePeriodSec: null, waveSourceStatus: "outside_coverage",
        station_or_gridpoint: `open-meteo-marine Gitterpunkt lat=${lat},lon=${lon}, Modell ${OM_MARINE_WAVE_MODEL} (dokumentiert ausserhalb der Modell-Abdeckung)` };
    }
    const dateStr = dt.toISOString().slice(0, 10);
  ...
  // NEU HI-2A (Batch Forecast, 31.08.2026), GETRENNT in HI-2A.1 (Hotfix, 31.08.2026 —
  // siehe OM_MARINE_WAVE_MODEL-Kommentar oben): Wassertemperatur als EIGENE Zeitreihe fuer einen
  // ganzen Zeitraum, OHNE "models"-Parameter (identisch zur bewiesenermassen funktionierenden
  // Einzelstunden-Methode getWaterTemp() oben) — SST braucht kein erzwungenes Wellenmodell und
  // darf durch die Wellenmodell-Wahl nicht mehr beeinflusst werden (Root Cause der
  // HI-2A.1-Regression, siehe Kommentar oben). `waterTempSourceStatus` dokumentiert transparent,
  // warum ein Wert fehlt (analog zu waveSourceStatus).
  async getWaterTempRangeRaw(lat, lon, startDt, endDt) {
    const qs = `latitude=${lat}&longitude=${lon}&hourly=sea_surface_temperature` +
      `&start_date=${startDt.toISOString().slice(0, 10)}&end_date=${endDt.toISOString().slice(0, 10)}&timezone=UTC`;
    const res = await fetchJson(`${OM_MARINE_URL}?${qs}`, 15000);
    // waterTempModel wird auch in den Fehlerzweigen mitgegeben ("best_match", der bewusst NICHT
    // erzwungene Standard) — dokumentiert, WAS versucht wurde, auch wenn der Request scheitert
    // (analoges Prinzip wie waveModel in getWaveRangeRaw() unten, fuer Konsistenz).
    if (!res.ok) return { ok: false, error: res.error, waterTempSourceStatus: "request_failed", waterTempModel: "best_match" };
    try {
      const timesIso = res.data.hourly.time;
      if (!timesIso.length) return { ok: true, times: [], timesIso: [], waterTempC: [], waterTempSourceStatus: "timestamp_missing", waterTempModel: "best_match" };
      const times = timesIso.map((t) => Date.parse(t + "Z"));
      const waterTempC = res.data.hourly.sea_surface_temperature;
      const anyTemp = Array.isArray(waterTempC) && waterTempC.some((v) => v !== null && v !== undefined);
      return {
        ok: true, times, timesIso, waterTempC,
        waterTempSourceStatus: anyTemp ? "available" : "provider_null",
        waterTempModel: "best_match", // bewusst KEIN erzwungenes Modell (siehe Kommentar oben)
        station_or_gridpoint: `open-meteo-marine Gitterpunkt lat=${lat},lon=${lon} (Batch-Zeitreihe, kein erzwungenes Modell)`,
      };
    } catch (e) {
      return { ok: false, error: `Parse-Fehler: ${e.message}`, waterTempSourceStatus: "request_failed", waterTempModel: "best_match" };
    }
  }

  // NEU HI-2A (Batch Forecast, 31.08.2026), Wellenmodell korrigiert in HI-2A.1 (siehe
  // OM_MARINE_WAVE_MODEL-Kommentar oben): Wellen als EIGENE Zeitreihe, GETRENNT von SST — direkte
  // Voraussetzung fuer buildHourlyForecastSeries() in hourly-intelligence.js (Auftrag Abschnitt
  // 3/16: "keine 120×N HTTP Requests"). getWaterTemp()/getWaveData() (Einzelstunde) bleiben
  // unveraendert bestehen, diese Methode ist rein additiv.
  async getWaveRangeRaw(lat, lon, startDt, endDt) {
    if (_isOutsideKnownCoverage(OM_MARINE_WAVE_MODEL, lat)) {
      return { ok: true, times: [], timesIso: [], waveHeightM: [], waveDirectionDeg: [], wavePeriodSec: [],
        waveSourceStatus: "outside_coverage", waveModel: OM_MARINE_WAVE_MODEL,
        station_or_gridpoint: `open-meteo-marine Gitterpunkt lat=${lat},lon=${lon}, Modell ${OM_MARINE_WAVE_MODEL} (dokumentiert ausserhalb der Modell-Abdeckung)` };
    }
    const qs = `latitude=${lat}&longitude=${lon}&hourly=wave_height,wave_direction,wave_period` +
      `&models=${OM_MARINE_WAVE_MODEL}&start_date=${startDt.toISOString().slice(0, 10)}&end_date=${endDt.toISOString().slice(0, 10)}&timezone=UTC`;
    const res = await fetchJson(`${OM_MARINE_URL}?${qs}`, 15000);
    if (!res.ok) return { ok: false, error: res.error, waveSourceStatus: "request_failed", waveModel: OM_MARINE_WAVE_MODEL };
    try {
      const timesIso = res.data.hourly.time;
      if (!timesIso.length) return { ok: true, times: [], timesIso: [], waveSourceStatus: "timestamp_missing", waveModel: OM_MARINE_WAVE_MODEL };
      const times = timesIso.map((t) => Date.parse(t + "Z"));
      ...
      return {
        ...
        waveModel: OM_MARINE_WAVE_MODEL,
        station_or_gridpoint: `open-meteo-marine Gitterpunkt lat=${lat},lon=${lon}, Modell ${OM_MARINE_WAVE_MODEL} (Batch-Zeitreihe)`,
      };
    } catch (e) {
      return { ok: false, error: `Parse-Fehler: ${e.message}`, waveSourceStatus: "request_failed", waveModel: OM_MARINE_WAVE_MODEL };
    }
  }
}
...
window.FIProviders = {
  ...
  // Test-Hook (HI-2A.1): reine, deterministische Helper-Funktion fuer die dokumentierte
  // geografische Modell-Einschraenkung (siehe OM_MARINE_MODEL_COVERAGE-Kommentar oben) — exportiert
